	...
	*/
	while (HAPI_Sprites.Update())
	{
		...
		*/
		...
		//Getting keyboard data to detect when the path should be generated
		const HAPI_TKeyboardData &keyData = HAPI_Sprites.GetKeyboardData();
		if (keyData.scanCode['1'])
		{
			testMap.GeneratePath(0);
		}
		else if(keyData.scanCode['2'])
		{
			testMap.GeneratePath(1);
		}
		else if(keyData.scanCode['3'])
		{
			testMap.GeneratePath(2);
		}

		const HAPI_TMouseData &mouseData = HAPI_Sprites.GetMouseData();
		
		scrollOffset -= (mouseData.wheelMovement / 3);

		testMap.RenderMap(scrollOffset);
		if (mouseData.leftButtonDown)
		{
			testMap.SelectTile(mouseData.x, mouseData.y, scrollOffset);
		}
		...
		/*if (keyData.scanCode['D'])
		{
			...
			{
				HAPI_Sprites.RenderText(VectorI(0, i * 20), Colour255::BLACK, "X: " + std::to_string(Path[i].x) + ", Y: " + std::to_string(Path[i].y), 20);
			}
		}*/

		//HAPI_Sprites.RenderText(VectorI(0, 0), Colour255::BLUE, std::to_string(*pointer));
		...
		*/
		...
		sprite->GetTransformComp().SetPosition({ 120, 200 });
		sprite->GetTransformComp().SetScaling({ .5f, .5f });
